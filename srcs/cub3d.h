...
void			*load_img_getinfo(t_data *img_info,
					void *mlx, char *relative_path);
...
t_sprites		*put_sprites(t_vars *vars, t_sprites *sprite_list,
				t_data *tex_list, t_data *main_img);
...
int				write_bmp(int fd, t_bmpheader *head,
					t_data *img_file, int img_size);
...
int				free_int_tex(int ***tab, int len,
					t_data *tex_list, t_vars *vars);
void			*free_int_tex_nl(int ***tab, int len, t_data *tex_list,
					t_vars *vars);
int				tex_chooser(t_ray *ray);
